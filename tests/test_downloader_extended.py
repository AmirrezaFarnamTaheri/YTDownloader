...
class TestDownloaderRobustness(unittest.TestCase):
    ...
    def test_get_video_info_download_error_fallback(self, mock_ydl):
        ...
        with patch('downloader.GenericExtractor.extract') as mock_generic:
            mock_generic.return_value = {'title': 'Fallback Generic', 'video_streams': []}
    ...
    def test_download_video_force_generic(self, mock_ydl):
        ...
        with patch('downloader.GenericExtractor.extract') as mock_generic, \
             patch('downloader.download_generic') as mock_download:

            mock_generic.return_value = {'title': 'Forced', 'video_streams': [{'url': 'http://d.com', 'ext': 'mp4'}]}
    ...
    def test_download_video_telegram(self, mock_ydl):
        ...
        with patch('downloader.TelegramExtractor.is_telegram_url', return_value=True), \
             patch('downloader.TelegramExtractor.extract') as mock_extract, \
             patch('downloader.download_generic') as mock_download:

             mock_extract.return_value = {'title': 'Tel', 'video_streams': [{'url': 'http://t.me/v', 'ext': 'mp4'}]}
