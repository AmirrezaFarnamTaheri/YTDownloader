...
def download_video(
    ...
    if start_time and end_time:
        try:
            ...
            if start_sec is None or end_sec is None: raise ValueError("Could not parse time format")
    ...
    if recode_video:
        postprocessors.append({'key': 'FFmpegVideoConvertor', 'preferedformat': recode_video})
    ...
    if gpu_accel and gpu_accel.lower() != 'none':
        ffmpeg_args = []
        if gpu_accel == 'cuda': ffmpeg_args.extend(['-c:v', 'h264_nvenc', '-preset', 'fast'])
        elif gpu_accel == 'vulkan': ffmpeg_args.extend(['-c:v', 'h264_vaapi'])
        if ffmpeg_args: ydl_opts['postprocessor_args'] = {'ffmpeg': ffmpeg_args}
    ...
    if cookies_from_browser:
        ydl_opts['cookies_from_browser'] = (cookies_from_browser, cookies_from_browser_profile if cookies_from_browser_profile else None)
